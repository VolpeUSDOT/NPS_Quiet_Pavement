...
```{r coef_table_macro}
# residuals(Feq.man.macro)
# str(Feq.man.macro)
kable(macro.coeff,
      caption = "Coefficients of Macro Pavement Type Manova",
      digits = 3) %>% 
  kable_styling(bootstrap_options = c("striped", "hover")) 
```

# Post-hoc ANOVAs by frequency
...
LZFeq_Chip.Seal.1.4 = ggplot()+
  ...
  scale_x_log10()+
  labs(title = "LZFeq ANOVA 1/4 in Chip Seal Coefficients", 
       x = "Frequency (Hz)", 
...
for (freq in names(LZFMax_l[16:41])){
  ...
  an.results = lm(LZFMax_l[[freq]] ~ -1 + macro_treatment + pavetemp * speed + Site + Year, data = LZFMax_l)
  frequencies = rbind(frequencies, c(as.numeric(substr(freq, 2, nchar(freq)))))
