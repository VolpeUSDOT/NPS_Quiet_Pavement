...
The below plots (figures 31-24 in the three-year report) illustrate the current difference in spectral LZFeq from pre-treatment, after three years of use. The plots are generated from the output of the ANOVA models based on 2021 measured data, holding vehicle speed constant at 50 mph and pavement temperature constant at 90 degrees Fahrenheit to normalize the measured data for facilitation of direct comparison of surface treatments. The change in LZFeq after three years is calculated as the difference between the predicted sound levels at 50 mph and 90 deg F for a particular treatment in 2021, compared to the baseline pavement conditions in 2016 under the same vehicle speed and pavement temperature conditions. The predicted values and standard errors at these conditions are saved as `LZFeq_2021_ANOVA_PredictedVals.csv` and `LZFeq_2021_ANOVA_PredictedVals_StdErr.csv`
...
LZFeq_Chip.Seal.1.4 = ggplot()+
  ...
  theme_bw(base_line_size = 1,
           base_size = 20)+
  scale_x_log10()+
  labs(title = 'Three-Year LZFeq ANOVA 1/4" Chip Seal Difference from Pre-treatment', 
       x = "Frequency (Hz)", 
       y = "LZFeq Difference \nfrom Pre-Treatment (dB)")  + 
  geom_hline(yintercept = 0) +
  ylim(y_range[1], y_range[2])
...
LZFeq_Chip.Seal.3.8 = ggplot()+
  ...
  theme_bw(base_line_size = 1,
           base_size = 20)+
  scale_x_log10()+
  labs(title = 'Three-Year LZFeq ANOVA 3/8" Chip Seal Difference from Pre-treatment', 
       x = "Frequency (Hz)", 
       y = "LZFeq Difference \nfrom Pre-Treatment (dB)")  + 
  geom_hline(yintercept = 0)+
  ylim(y_range[1], y_range[2])
...
LZFeq_Microsurfacing.II = ggplot()+
  ...
  theme_bw(base_line_size = 1,
           base_size = 20)+
  scale_x_log10()+
  labs(title = "Three-Year LZFeq ANOVA Type II Microsurfacing Difference from Pre-treatment", 
       x = "Frequency (Hz)", 
       y = "LZFeq Difference \nfrom Pre-Treatment (dB)")  + 
  geom_hline(yintercept = 0)+
  ylim(y_range[1], y_range[2])
...
LZFeq_Microsurfacing.III = ggplot()+
  ...
  theme_bw(base_line_size = 1,
           base_size = 20)+
  scale_x_log10()+
  labs(title = "Three-Year LZFeq ANOVA Type III Microsurfacing Difference from Pre-treatment", 
       x = "Frequency (Hz)", 
       y = "LZFeq Difference \nfrom Pre-Treatment (dB)")  + 
  geom_hline(yintercept = 0)+
  ylim(y_range[1], y_range[2])
...
point_plot <- ggplot(predvals, aes(x = frequency,
  ...
  theme_bw(base_line_size = 1,
           base_size = 15) +
  scale_x_log10() +
  labs(title = "Three-Year LZFeq by Surface Treatment \nat 50mph vehicle speed & 90degF pavement temp", 
       x = "Frequency (Hz)", 
       y = "LZFeq (dB)")
